...
export function ConversationPanel({ messages }: { messages: Message[] }) {
  return (
    <div className="p-6 flex flex-col h-full bg-gray-100/50">
      <div className="flex-1 space-y-4 overflow-y-auto">
        {messages.map((msg, index) => (
          <div key={index} className={cn("flex", msg.from === 'Sally' ? "justify-end" : "justify-start")}>
            <div className={cn(
              ...
              {msg.flag === 'needs_touch' && (
                <motion.div 
                    ...
                    className="mt-3 p-3 bg-amber-100 border border-amber-200 rounded-lg">
                  <div className="flex items-start gap-2">
                    <AlertTriangle className="size-4 text-amber-600 mt-0.5"/>
                    <div>
                      <p className="font-semibold text-amber-800">Needs your touch</p>
                      <p className="text-xs text-amber-600">Sally needs help with this reply.</p>
             ...
             {msg.from === 'Sally' && <Bot className="size-5 ml-2 mt-1 text-primary/70 flex-shrink-0"/>}
          </div>
        ))}
      </div>
    </div>

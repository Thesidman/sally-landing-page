    ...
    <section className="py-20 sm:py-32 bg-white/50">
      <div className="container mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto">
          <Badge variant="secondary" className="mb-3 text-primary/80 bg-primary/10 border-primary/20">
            Why Sally
          </Badge>
...
const Card1Visual = () => {
    const convos = [
        { name: "Ankit", status: "Nurturing", color: "accent" as const, avatarSeed: "10", time: "2m ago" },
        { name: "Sarah L.", status: "Booked", color: "primary-ghost" as const, avatarSeed: "11", time: "1h ago" },
        { name: "Michael P.", status: "Waiting", color: "outline" as const, avatarSeed: "12", time: "3h ago" },
        { name: "Jessica B.", status: "High intent", color: "destructive" as const, avatarSeed: "13", time: "1d ago" },
    ];
    return (
        <Card className="bg-white/70 backdrop-blur-sm p-3 rounded-xl shadow-inner-white border-white/50">
            ...
            <div className="space-y-2">
                {convos.map((c) => (
                    <div key={c.name} className="flex items-center bg-white/80 p-2 rounded-md shadow-sm">
                        <Avatar className="size-6 mr-2">
                            <AvatarImage src={`https://picsum.photos/seed/${c.avatarSeed}/24/24`} alt={c.name} />
            ...
            <div className="flex items-center justify-between space-x-1">
                {steps.map((step, i) => (
                    <React.Fragment key={step}>
                        <div className={cn(
                            "flex-1 py-1 px-1 rounded-md text-center",
                            i <= 1 ? "bg-primary/20 text-primary" : "bg-gray-100 text-muted-foreground"
                        )}>
    ...
    <Card className="bg-white/70 backdrop-blur-sm p-3 rounded-xl shadow-inner-white border-white/50 flex items-center gap-4">
        ...
        <div className="flex-1 grid grid-cols-2 gap-1.5 text-center">
            <div className="bg-white/80 p-1.5 rounded-md shadow-sm">
                <p className="text-[10px] font-medium text-foreground">Replies Sent</p>
            </div>
            <div className="bg-white/80 p-1.5 rounded-md shadow-sm">
                <p className="text-[10px] font-medium text-foreground">Meetings Booked</p>
            </div>
            <div className="bg-white/80 p-1.5 rounded-md shadow-sm">
                <p className="text-[10px] font-medium text-foreground">Issues Resolved</p>
            </div>
            <div className="bg-white/80 p-1.5 rounded-md shadow-sm">
                <p className="text-[10px] font-medium text-foreground">Follow-ups</p>
            </div>
        </div>
    </Card>
);
...
const Card5Visual = () => {
    const items = [
        ...
        { text: "FAQ answered", status: "Done", icon: <Check className="size-3" /> },
        { text: "Trial extended", status: "Running", icon: <Zap className="size-3" /> },
    ];
    return (
        <Card className="bg-white/70 backdrop-blur-sm p-3 rounded-xl shadow-inner-white border-white/50">
            ...
            <div className="space-y-1.5">
                {items.map(item => (
                    <div key={item.text} className="flex items-center bg-white/80 p-1.5 rounded-md shadow-sm text-xs">
                        <div className={cn("size-5 flex items-center justify-center rounded-sm mr-2",
                          item.status === 'Done' ? 'bg-primary/20 text-primary' : 'bg-accent/80 text-accent-foreground'
                        )}>
                          {item.icon}
                        </div>
